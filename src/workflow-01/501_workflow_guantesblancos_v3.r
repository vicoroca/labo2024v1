...
TS_strategy_guantesblancos_202109 <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$train$undersampling <- 0.2 #aumento la cantidad de los continua
...
HT_tuning_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$lgb_param <- list(
    boosting = "gbdt", # puede ir  dart  , ni pruebe random_forest #V-> vuelvo con gbdt ya que es mas rapido
    objective = "binary",
    metric = "custom",
    ...
    learning_rate = c( 0.01, 0.9 ), #cambie
    feature_fraction = c( 0.6, 0.9 ), #cambie
    num_leaves = c( 400L, 1200L,  "integer" ), #cambie
    min_data_in_leaf = c( 200L, 3000L, "integer" ) #cambie
  )


  # una Beyesian de Guantes Blancos, solo hace 15 iteraciones
  param_local$bo_iteraciones <- 30 # iteraciones de la Optimizacion Bayesiana #cambio a 30 iteraciones ya que las primeras 16 no son utiles.
...
ZZ_final_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$kaggle$envios_desde <-  10000L #saco los de 9500 por que siempre me dieron ganancia baja.
  ...
  param_local$qsemillas <- 10 #agrego 5 semillas mas
...
corrida_guantesblancos_202109 <- function( pnombrewf, pvirgen=FALSE )
{
  ...
  ZZ_final_guantesblancos( "ZZ00013", c("HT0003","TS0003") )
...
corrida_guantesblancos_202107 <- function( pnombrewf, pvirgen=FALSE )
{
  ...
  # El ZZ depente de HT y TS
  ZZ_final_guantesblancos( "ZZ00023", c("HT0003", "TS0003") )
